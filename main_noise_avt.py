...
from dataset.MOSIDataset import MOSIDataset_mask
...
def get_arguments():
    ...
    parser.add_argument('--dataset', default='MOSI', type=str,
                        help='VGGSound, KineticSound, CREMAD, MOSI')
    ...
    parser.add_argument('--cylcle_epoch', default=10, type=int)     # 10,20,40
    parser.add_argument('--num_freeze', default=5, type=int)
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler,scheduler_warmup=None,
    ...
    for step, data in enumerate(tqdm(dataloader, desc="Epoch {}/{}".format(epoch, args.epochs))):
        if args.dataset in ['MOSI']:
            spec, image, text_data, label, audio_variance, visual_variance, text_variance = data
        ...
        loss_cls = loss_f + (loss_a  + loss_v + loss_t) * args.gamma
        ...
        if not isinstance(t_mul, int):
            regurize_t = regurize(t_mul, t_std,target_var=t_variance)
            regurize_t = regurize_t.cuda()
        else:
            regurize_t = torch.zeros(1).float().cuda()
            t_std = torch.zeros(1).float().cuda()
        ...
        regurize_loss = (regurize_a + regurize_v + regurize_t)
        ...
        variance_fc_loss = F.mse_loss(a_std_fc,a_variance) + F.mse_loss(v_std_fc,v_variance) + F.mse_loss(t_std_fc,t_variance)
        ...
        if step % 100 == 0:
            ...
            print("regurize_Loss:", regurize_loss.item(), "unimodal_loss:", (loss_a + loss_v + loss_t).item(), "cls_loss:",
        ...
        if step % 100 == 0:
            ...
            print("variance:",a_std.mean().item(),v_std.mean().item(),t_std.mean().item(),a_std_fc.mean().item(),v_std_fc.mean().item(),t_std_fc.mean().item(),a_variance.mean(),v_variance.mean(),t_variance.mean())
...
def valid(args, model, device, dataloader):
    ...
    with torch.no_grad():
        ...
        for step, (spec, image, text_data, label, audio_variance, visual_variance, text_variance) in enumerate(dataloader):
            if args.dataset in ['MOSI']:
                text_data = {k: v.to(device) for k, v in text_data.items()}
                spec = spec.to(device)
                image = image.to(device)
...
def main():
    ...
    for name, param in model.text_net.named_parameters():
        ...
        for i in range(args.num_freeze):  
    ...
    if args.optimizer == 'sgd':
        optimizer = optim.SGD(model.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=1e-4)  # 1e-4
    ...
    if args.warmup:
        from models.lib.model_develop_utils import GradualWarmupScheduler
    ...
    if args.dataset == 'MOSI':
        train_dataset_noise = MOSIDataset_mask(args, mode='train',add_noise=True)
        test_dataset = MOSIDataset_mask(args, mode='valid',add_noise=False)
        test_dataset_noise5 = MOSIDataset_mask(args, mode='valid', add_noise=True)
    ...
    log_path = os.path.join(args.ckpt_path, 'Noise_freeze_' + str(args.num_freeze) + '_w20' + str(args.num_frame) + '_' + args.dataset + 'gamma' + str(args.gamma) + '_'  + args.fusion_method + '_' + args.modality + '_' + '.csv')
    print(log_path)
    ...
    if args.train:
        acc = 0
        acc5, acc_a5, acc_v5, acc_t5 = 0, 0, 0, 0
        acc_clean, acc_a_clean, acc_v_clean, acc_t_clean = 0, 0, 0, 0
        ...
        for epoch in range(args.epochs):
            ...
            if args.use_tensorboard:
                ...
                if epoch > (args.epochs) * 0.7: 
                    acc5, acc_a5, acc_v5, acc_t5 = valid(args, model, device, test_dataloader_noise5)
                    acc_clean, acc_a_clean, acc_v_clean, acc_t_clean = valid(args, model, device, test_dataloader)
                ...
                with open(log_path, 'a+', newline='') as csvfile:
                    writer = csv.writer(csvfile, delimiter=",")
                    writer.writerow([acc, acc5, acc_a5, acc_v5, acc_t5, acc_clean, acc_a_clean, acc_v_clean, acc_t_clean])
