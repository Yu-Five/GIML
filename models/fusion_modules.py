...
class ConcatFusion_AUXI_3(nn.Module):
    def __init__(self, input_dim=1024, output_dim=100):
        super(ConcatFusion_AUXI_3, self).__init__()
        self.fc_out = nn.Linear(input_dim, output_dim)
    ...
    def forward(self, x, y, z):
        ...
        output = self.fc_out(output)
        x_out = self.fc_out(torch.cat((x, torch.zeros_like(y), torch.zeros_like(z)), dim=1))
        y_out = self.fc_out(torch.cat((torch.zeros_like(x), y, torch.zeros_like(z)), dim=1))
        z_out = self.fc_out(torch.cat((torch.zeros_like(x), torch.zeros_like(y), z), dim=1))
