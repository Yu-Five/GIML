...
from models.basic_model import AVClassifier_AUXI_AT
...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler,scheduler_warmup=None,
    ...
    for step, data in enumerate(tqdm(dataloader, desc="Epoch {}/{}".format(epoch, args.epochs))):
        if args.dataset in ['MOSI']:
            ...
            a, t, out, a_feature, t_feature, a_mul, a_std, t_mul, t_std, out_a, out_t, a_std_fc, t_std_fc = model(spec.unsqueeze(1).float(), text_data)
        ...
        calculate_b = torch.mean(torch.abs(out_t), 0).sum().cpu().detach()

        # print(out_a.shape,out_v.shape)



        loss_cls = loss_f + (loss_a + loss_t) * args.gamma
        ...
        regurize_loss = (regurize_a + regurize_t)
        ...
        variance_fc_loss = F.mse_loss(a_std_fc,a_variance) + F.mse_loss(t_std_fc,t_variance)
        ...
        if step % 100 == 0:
            ...
            print("regurize_Loss:", regurize_loss.item(), "unimodal_loss:", loss_a.item(), loss_t.item(), "cls_loss:",
        ...
        if step % 100 == 0:
            ...
            print("variance:",a_std.mean().item(),t_std.mean().item(),a_std_fc.mean().item(),t_std_fc.mean().item(),a_variance.mean(),t_variance.mean())
        ...
        if args.dataset in ['MOSI']:
            ...
            file_name = 'audio_text_grad_vanilla' + '.csv'
            with open(file_name, 'a', newline='') as f:
                writer = csv.writer(f)
                row = [audio_grad_sum, text_grad_sum]
                writer.writerow(row)
...
def main():
    ...
    model = AVClassifier_AUXI_AT(args)
    for name, param in model.text_net.named_parameters():
        if "embeddings" in name:
            param.requires_grad = False
        
        for i in range(9):   # 冻结 0~5（之前就是这个）、0~10、0~9 层、0~6层
    ...
    log_path = os.path.join(args.ckpt_path, 'AT' + str(args.num_frame) + '_' + args.dataset + 'gamma' + str(args.gamma) + '_'  + args.fusion_method + '_' + args.modality + '_' + '.csv')
    print(log_path)
        ...
        for epoch in range(args.epochs):
            ...
            if args.use_tensorboard:
                ...
                if epoch > (args.epochs) * 0.3: 
                    acc5, acc_a5, acc_t5 = valid(args, model, device, test_dataloader_noise5)
                    acc_clean, acc_a_clean, acc_t_clean = valid(args, model, device, test_dataloader)
                ...
                with open(log_path, 'a+', newline='') as csvfile:
                    writer = csv.writer(csvfile, delimiter=",")
                    writer.writerow([acc, acc5, acc_a5, acc_t5, acc_clean, acc_a_clean, acc_t_clean])
